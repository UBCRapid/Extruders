%inductance
figure();
title("Inductance versus frequency and temperature");
ylabel("Coil inductance (Henries)");
xlabel("Frequency (Hz)");

axis_xmin = min(freqrange);
axis_xmax = max(freqrange);
axis_ymin = 0;
axis_ymax = max(max(inductance));
...
legend_ymin = 0.1*axis_ymax;
legend_ymax = 0.5*axis_ymax;
...
%	colour_imag = [ (index1/numtemps), 0.5, 0];
	...
	%Prepare the data
	inductanceplot = inductance(:,index1);

	% plot the graphs
	semilogx(freqrange,inductanceplot,"linewidth",2,"color",colour_mag)
