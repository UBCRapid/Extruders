%reflect resistance
figure();
title("Reflected resistance versus frequency and temperature");
ylabel("Reflected resistance (Ohms)");
xlabel("Frequency (Hz)");

axis_xmin = min(freqrange);
axis_xmax = max(freqrange);
axis_ymin = 0;
axis_ymax = max(max(reflect_resistance));
...
%	colour_imag = [ (index1/numtemps), 0.5, 0];
	...
	%Prepare the data
	reflect_resistanceplot =reflect_resistance(:,index1);

	% plot the graphs
	semilogx(freqrange,reflect_resistanceplot,"linewidth",2,"color",colour_mag)
